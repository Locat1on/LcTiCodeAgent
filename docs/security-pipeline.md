...
`replace_in_file`要求 `expected_sha256`：`read_file`结果首行返回整个文件的SHA-256，模型编辑时必须回传该值。执行前重新计算当前文件哈希，与读取时不一致则拒绝本次编辑并要求重新读取。这与Git preflight的状态令牌是同一种状态绑定语义，用于消除普通文件编辑在决策与执行之间的TOCTOU窗口。
